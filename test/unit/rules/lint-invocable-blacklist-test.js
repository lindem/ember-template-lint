...
generateRuleTests({
  ...
  good: ['{{baz}}', '{{baz foo=bar}}', '{{baz foo=(baz)}}', '{{#baz}}{{/baz}}',
    '{{#baz foo=bar}}{{/baz}}', '{{#baz foo=(baz)}}{{/baz}}',
    '{{component "baz"}}', '{{component "baz" foo=bar}}',
    '{{component "baz" foo=(baz)}}', '{{#component "baz"}}{{/component}}',
    '{{#component "baz" foo=bar}}{{/component}}',
    '{{#component "baz" foo=(baz)}}{{/component}}',
    '{{yield (component "baz")}}', '{{yield (component "baz" foo=bar)}}',
    '{{yield (component "baz" foo=(baz))}}', '{{yield (baz (baz (baz) bar))}}',
...
generateRuleTests({
  ...
  config: {
    foo: {
      withAttributes: ['experimental']
    }
  },
  good: ['{{foo}}'],
  bad: [/*{
    template: '{{foo experimental=true}}',

    result: {
      message: 'Cannot use blacklisted combination of helper or component \'{{foo}}\' with named parameter \'experimental\'',
      moduleId: 'layout.hbs',
      source: '{{foo}}',
      line: 1,
      column: 0
